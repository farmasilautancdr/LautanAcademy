...
function start() {
  error.value = ''
  if (!selectedTopic.value) { error.value = 'Pick a module first.'; return }
  if (alreadyAttemptedToday.value) { error.value = `You've already attempted "${selectedTopic.value}" today. Try again tomorrow.`; return }
  const questions = shuffle(allQuestions.value.filter((q) => q.topic === selectedTopic.value))
  if (!questions.length) { error.value = 'No questions available for this module yet.'; return }
...
<template>
  <div class="min-h-screen bg-seafoam">
    <header class="bg-deepsea px-6 py-5">
      ...
      <h1 class="font-display text-xl font-semibold text-white">Module Quiz</h1>
    </header>
    ...
    <main class="max-w-3xl mx-auto px-6 py-8">
      ...
      <div v-else class="bg-white rounded-xl2 p-6 shadow-sm space-y-4">
        <div>
          <label class="block text-sm font-medium text-ink mb-1">Module</label>
        ...
        <p v-if="selectedTopic" class="text-slate text-sm">
          {{ bankCount }} question{{ bankCount === 1 ? '' : 's' }} in this module.
        </p>
