          ...
          <details v-for="h in paginatedHistory" :key="h.AttemptID" class="px-5 py-3">
            ...
            <div v-if="wrongsFor(h.AttemptID).length" class="mt-3 space-y-2">
              <div v-for="(w, j) in wrongsFor(h.AttemptID)" :key="j" class="bg-seafoam rounded-lg p-3">
                <p class="text-xs font-medium text-coral">{{ t('warehouseManagerResultsView.questionPrefix', { text: bilingual(w, 'Question Text') }) }}</p>
                <p class="text-xs text-aqua font-semibold mt-1">{{ t('warehouseManagerResultsView.correctLabel', { text: bilingual(w, 'Correct Answer') }) }}</p>
              </div>
            </div>
          </details>
